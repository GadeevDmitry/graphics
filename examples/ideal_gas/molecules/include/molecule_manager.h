...
public:
    explicit molecule_manager_t(const double molecule_size_, const size_t molecules_num_, const vec2d &area_size_);
    ~molecule_manager_t() { vector_dtor(&molecules); }

    const sf::Texture &get_texture() const { return molecules_tex.getTexture(); }

    void draw();
    void refresh(const double frame_time, const scene_t &scene);
};
