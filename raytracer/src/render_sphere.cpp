...
bool render_sphere_t::get_intersection(const ray_t &ray, vec3d &intersect_pt) const
{
    ...
    int square_solve = square_solver(ray.get_direction().len2(), 2*(ray.get_direction(), distance), distance.len2() - radius*radius, t_1, t_2);

    if (square_solve != 2 || t_1 <= 0 || t_2 <= 0) return false;

    double t = t_1 < t_2 ? t_1 : t_2;
    intersect_pt = ray.get_start() + t*ray.get_direction();
